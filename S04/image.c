...
void ecrire_image(Image I){
	for (UINT h = 1; h < hauteur_image(I);h++){
		for (UINT l = 1; l < largeur_image(I);l++){
			Pixel pix = get_pixel_image(I, l, h);
			if (pix){
				printf("■");
			}
			else{
...
Image negatif_image(Image I){
	UINT Il = largeur_image(I);
	UINT Ih = hauteur_image(I);
	Image Ineg = creer_image(Il, Ih);
	for (UINT h = 1; h < Ih; h++){
		for (UINT l = 1; l < Il; l++){
			Pixel pix = get_pixel_image(I, l, h);
			if (!pix){
				set_pixel_image(Ineg, l, h, NOIR);
			}
		}
	}
	return Ineg;
